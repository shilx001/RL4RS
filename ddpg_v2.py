...
# use conv_lstm for state representation.
...
class Actor(object):
    def __init__(self, sess, state_dim, action_dim, action_bound, learning_rate, tau, batch_size, hidden_size,
        ...
        self.inp = tf.placeholder(shape=[None, self.max_seq_length, self.s_dim], dtype=tf.float32,
    ...
    def create_actor_network(self, scope, reuse=False):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            states, _ = tf.nn.dynamic_rnn(basic_cell, s, dtype=tf.float32, sequence_length=self.length)
...
class Critic(object):
    ...
    def create_critic_network(self, scope, actions, reuse=False):
        with tf.variable_scope(scope, reuse=reuse):
            basic_cell = tf.contrib.rnn.BasicLSTMCell(num_units=self.hidden_size)
            outputs, _ = tf.nn.dynamic_rnn(basic_cell, self.inp, dtype=tf.float32, sequence_length=self.length)
            features = tf.contrib.slim.conv2d(outputs, num_outputs=self.hidden_size, kernel_size=3)
            max_pool = tf.reduce_max(features, axis=1)
            mean_pool = tf.reduce_mean(features, axis=1)
            states = tf.concat([max_pool, mean_pool], axis=1)
            input_s = tf.reshape(states, [-1, 2*self.hidden_size])
